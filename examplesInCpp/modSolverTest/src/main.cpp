...
#include <syst/priv/CmdLine.h>
#include <mem/PointerReleaser.h>

fo::fs::path s_homePath;
...
td::String getModlInput(const char* fileName, Location location)
{
    ...
    {
        case Location::Real:
            path = s_homePath / "other_bin/TestData/modSolver/real" / fileName;
            break;
            
        case Location::Complex:
            path = s_homePath / "other_bin/TestData/modSolver/cmplx" / fileName;
            break;
        case Location::Selected:
            path = s_homePath / "other_bin/TestData/modSolver/PowerSystem/PaperExamples" / fileName;
            break;
    }
...
td::String getOutput(const char* fileName)
{
#ifdef MU_WINDOWS
    fo::fs::path ramDisk = "R:/";
#else
    fo::fs::path ramDisk = "/Volumes/RAMDisk";
#endif
    fo::fs::path path = ramDisk / fileName;
    td::String str = path.string();
    return str;
}
